...
def get_event_data(values):
    ...
    for row in values:
        ...
        if "resume" in row[source_event_event_index].lower():
            ...
            source_event_data[row[source_event_email_index].lower()].has_professional = 1
        elif "pss" in row[source_event_event_index].lower():
            source_event_data[row[source_event_email_index].lower()].has_professional = 1
...
def update_mentor_total_log_data(service, values):
    ...
    # for each mentor log
    for row in values:
        mentor_point_dict[row[source_mentor_email_index].lower()] = mentor_point_dict.get(row[source_mentor_email_index].lower(), 0) + 1 
